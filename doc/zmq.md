...
The Morrigan Core daemon can be configured to act as a trusted "border
router", implementing the morrigan wire protocol and relay, making
...
The ZeroMQ feature in Morrigan Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building morrigand:
...
For instance:
    ...
    $ morrigand -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/morrigand.tx.raw
...
These options can also be provided in morrigan.conf.
...
From the perspective of morrigand, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into morrigand directly. Furthermore, no information is
...
using. Morrigand appends an up-counting sequence number to each
