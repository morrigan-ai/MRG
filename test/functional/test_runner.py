...
`test/functional/test_framework/test_framework.py:MorriganTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/morrigan_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and morrigand
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_morrigand = config["components"].getboolean("ENABLE_MORRIGAND")
    if not (enable_wallet and enable_cli and enable_morrigand):
        print("No functional tests to run. Wallet, utils, and morrigand must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if morrigand is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "morrigand"]) is not None:
            print("%sWARNING!%s There is already a morrigand process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "MORRIGAND" not in os.environ:
        os.environ["MORRIGAND"] = build_dir + '/src/morrigand' + exeext
        os.environ["MORRIGANCLI"] = build_dir + '/src/morrigan-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `morrigan-cli help` (`rpc_interface.txt`).
