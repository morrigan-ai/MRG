...
#ifndef MORRIGAN_PROTOCOL_H
#define MORRIGAN_PROTOCOL_H
...
/**
 * Morrigan protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // set by all Morrigan Core nodes, and is unset by SPV clients or other peers that just want
    // network services but don't provide them.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // Morrigan Core does not support this but a patch set called Morrigan XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // Morrigan Core nodes used to support this by default, without advertising this bit,
    ...
    // morrigan-development mailing list. Remember that service bits are just
