...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetMRG) { // Looking for MRG Tx OutPoints Only
                    if (fMRGLimitHit) // We hit our limit
                        continue;

                    // We only want MRG OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-morrigan-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** MRG START */
                    // Check to see if you need to make an asset data outpoint OP_MRG_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** MRG START */ /** MRG END */
                    {
