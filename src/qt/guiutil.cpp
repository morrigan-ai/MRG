...
#include "morriganaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Morrigan address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new MorriganAddressEntryValidator(parent));
    widget->setCheckValidator(new MorriganAddressCheckValidator(parent));
}
...
bool parseMorriganURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no morrigan: URI
    if(!uri.isValid() || uri.scheme() != QString("morrigan"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MorriganUnits::parse(MorriganUnits::MRG, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseMorriganURI(QString uri, SendCoinsRecipient *out)
{
    // Convert morrigan:// to morrigan:
    //
    //    Cannot handle this later, because morrigan:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("morrigan://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "morrigan:");
    }
    QUrl uriInstance(uri);
    return parseMorriganURI(uriInstance, out);
}

QString formatMorriganURI(const SendCoinsRecipient &info)
{
    QString ret = QString("morrigan:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MorriganUnits::format(MorriganUnits::MRG, info.amount, false, MorriganUnits::separatorNever));
        paramCount++;
    }
...
bool openMorriganConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(MORRIGAN_CONF_FILENAME);
    ...
    /* Open morrigan.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Morrigan.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Morrigan (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Morrigan (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Morrigan*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "morrigan.desktop";
    return GetAutostartDir() / strprintf("morrigan-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a morrigan.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Morrigan\n";
        else
            optionFile << strprintf("Name=Morrigan (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the morrigan app
...
bool GetStartOnSystemStartup()
{
    CFURLRef morriganAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (morriganAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, morriganAppUrl);

    CFRelease(morriganAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add morrigan app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, morriganAppUrl, nullptr, nullptr);
    }
