...
const std::string MorriganGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MorriganGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "MRGBTC"  , 1,          8},
    { "mBTC",   "MRGBTC"  , 1000,       5},
    { "µBTC",   "MRGBTC"  , 1000000,    2},
    { "Satoshi","MRGBTC"  , 100000000,  0},
    { "USDT",   "MRGUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(MorriganGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

MorriganGUI::MorriganGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void MorriganGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Morrigan address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and morrigan: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MRG addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Morrigan addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Morrigan addresses"));
    ...
    openAction->setStatusTip(tr("Open a morrigan: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Morrigan command-line options").arg(tr(PACKAGE_NAME)));
...
void MorriganGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void MorriganGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/MRGtext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/Morrigancointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* morriganLabelLayout = new QVBoxLayout(toolbarWidget);
        morriganLabelLayout->addWidget(labelToolbar);
        morriganLabelLayout->addWidget(m_toolbar);
        morriganLabelLayout->setDirection(QBoxLayout::TopToBottom);
        morriganLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Morrigancoin Market Price"));
        ...
        comboMRGUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboMRGUnit->addItems(list);
        comboMRGUnit->setFixedHeight(26);
        comboMRGUnit->setContentsMargins(5,0,0,0);
        comboMRGUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboMRGUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/morrigan-ai/MRG/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboMRGUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboMRGUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Morrigancoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/morrigan-ai/MRG/releases";
...
void MorriganGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/MRGtext")));
        m_toolbar->setMaximumWidth(65);
...
void MorriganGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MorriganGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MorriganGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MorriganGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MorriganGUI::setWalletActionsEnabled(bool enabled)
{
...
void MorriganGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MorriganGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MorriganGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MorriganGUI::optionsClicked()
{
...
void MorriganGUI::showDebugWindow()
{
...
void MorriganGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MorriganGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MorriganGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MorriganGUI::openClicked()
{
...
void MorriganGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MorriganGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MorriganGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MorriganGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MorriganGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MorriganGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MRG START */
void MorriganGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MorriganGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MorriganGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void MorriganGUI::gotoRestrictedAssetsPage()
{
...
void MorriganGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Morrigan network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MorriganGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MorriganGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MorriganGUI::updateHeadersSyncProgressLabel()
{
...
void MorriganGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MorriganGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Morrigan"); // default title
...
void MorriganGUI::changeEvent(QEvent *e)
{
...
void MorriganGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MorriganGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MorriganGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MRG")
        msg += tr("Amount: %1\n").arg(MorriganUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MorriganUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MorriganGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MorriganGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MorriganGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MorriganGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MorriganGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void MorriganGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MorriganGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MorriganGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MorriganGUI::detectShutdown()
{
...
void MorriganGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MorriganGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MorriganGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(MorriganGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void MorriganGUI::subscribeToCoreSignals()
{
...
void MorriganGUI::unsubscribeFromCoreSignals()
{
...
void MorriganGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MorriganGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MorriganUnits::Unit> units = MorriganUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MorriganUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(MorriganUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(MorriganUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (MorriganUnits::Unit u : MorriganUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MorriganUnits::name(u)), this);
...
{
    setText(MorriganUnits::name(newUnits));
}
...
void MorriganGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MorriganGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "MorriganGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboMRGUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void MorriganGUI::getPriceInfo()
{
...
void MorriganGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/MorriganProject/Morrigancoin/releases"));
