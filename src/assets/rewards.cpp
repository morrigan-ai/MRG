...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for MRG
    ...
    if (p_rewardSnapshot.strDistributionAsset != "MRG") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is MRG with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using MRG differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "MRG") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Morrigan address (already validated during ownership snapshot creation)
