...
The RPM spec file provided here is for Morrigan-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://morrigan.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Morrigan git repository](https://github.com/morrigan-ai/MRG) but are not part of
...
Currently only `Source10` is used, for the example `morrigan.conf` file.
...
in the morrigan git repository. At present only an SVG version of the Morrigan
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the morrigan
...
needed when 0.12.1 is released, a proper fix is already in the Morrigan git
master branch.
...
idea to build Morrigan Core against a static version of that library compiled
...
The `morrigan.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `morrigan-server` package. At this point, most RPM based
...
The `morrigan-server` package should have SELinux support. How to properly do
...
Morrigan-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/morrigan/). He has been
packaging Morrigan for Fedora at least since 2012.
...
4. I renamed the `morrigan` package that contains the Qt GUI to `morrigan-core` as
that appears to be how the general population refers to it, in contrast to
`morrigan-xt` or `morrigan-classic`. I wanted to make sure the general population
